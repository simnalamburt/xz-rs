    ...
    loop {
        match coder.sequence {
            ...
            SEQ_STREAM_FOOTER => {
                ...
                let ret: lzma_ret = lzma_stream_flags_compare(&coder.stream_flags, &footer_flags);
                if ret != LZMA_OK {
                    return ret;
                }
...
unsafe fn stream_decoder_end(coder: &mut lzma_stream_coder, allocator: *const lzma_allocator) {
...
unsafe fn stream_decoder_get_check(coder: &lzma_stream_coder) -> lzma_check {
...
pub(crate) unsafe fn lzma_stream_decoder_init(
    ...
    if coder.is_null() {
        ...
        (*next).code = coder_code_fn!(stream_decode, lzma_stream_coder);
        (*next).end = coder_end_fn!(stream_decoder_end, lzma_stream_coder);
        (*next).get_check = coder_get_check_fn!(stream_decoder_get_check, lzma_stream_coder);
        (*next).memconfig = coder_memconfig_fn!(stream_decoder_memconfig, lzma_stream_coder);
