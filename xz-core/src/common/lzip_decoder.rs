...
unsafe fn lzip_decoder_end(coder: &mut lzma_lzip_coder, allocator: *const lzma_allocator) {
    lzma_next_end(::core::ptr::addr_of_mut!(coder.lzma_decoder), allocator);
    crate::alloc::internal_free(coder, allocator);
}
unsafe fn lzip_decoder_get_check(_coder: &lzma_lzip_coder) -> lzma_check {
    LZMA_CHECK_CRC32
}
