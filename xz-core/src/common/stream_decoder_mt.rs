...
unsafe extern "C" fn worker_decoder(thr_ptr: *mut c_void) -> *mut c_void {
    ...
    loop {
        ...
        ret = if (*thr).block_decoder.coder.is_some() {
            (*thr).block_decoder.code(
                worker_allocator(thr),
                (*thr).in_0,
    ...
    loop {
        ...
        match block_state {
            STREAM_MT_BLOCK_DIRECT_RUN => {
                ...
                if coder.block_decoder.coder.is_none() {
                    return LZMA_PROG_ERROR;
                }
                let ret_4: lzma_ret = coder.block_decoder.code(
                    allocator, in_0, in_pos, in_size, out, out_pos, out_size, action,
                );
            ...
            STREAM_MT_BLOCK_HEADER => {
                ...
                if ret_0 == LZMA_OK {
                    ...
                    coder.mem_next_filters = lzma_raw_decoder_memusage(::core::ptr::addr_of_mut!(
                        coder.filters
                    )
                        as *mut lzma_filter);
            ...
            STREAM_MT_INDEX_DECODE => {
                ...
                let ret_5: lzma_ret =
                    lzma_index_hash_decode(&mut *coder.index_hash, in_0, in_pos, in_size);
...
unsafe fn stream_decoder_mt_end(coder: &mut lzma_stream_coder, allocator: *const lzma_allocator) {
...
unsafe fn stream_decoder_mt_get_check(coder: &lzma_stream_coder) -> lzma_check {
            ...
            *progress_out = coder.progress_out;
            ...
            while i < coder.threads_initialized as size_t {
                ...
                while if mythread_i_1867 != 0 {
                    mythread_mutex_unlock(::core::ptr::addr_of_mut!((*coder.threads.add(i)).mutex));
                    0
                } else {
                    mythread_mutex_lock(::core::ptr::addr_of_mut!((*coder.threads.add(i)).mutex));
                    1
                } != 0
                {
...
unsafe fn stream_decoder_mt_init(
    ...
    coder = (*next).coder_as::<lzma_stream_coder>();
    if coder.is_null() {
...
impl NextCoder for lzma_stream_coder {
    ...
    unsafe fn end(&mut self, allocator: *const lzma_allocator) {
        stream_decoder_mt_end(self, allocator)
    }
    unsafe fn get_check(&self) -> Option<lzma_check> {
        Some(stream_decoder_mt_get_check(self))
    }
    unsafe fn memconfig(
        &mut self,
        memusage: *mut u64,
        ...
        Some(stream_decoder_mt_memconfig(
            self,
            memusage,
            old_memlimit,
            new_memlimit,
    ...
    unsafe fn get_progress(&mut self) -> Option<(u64, u64)> {
        ...
        stream_decoder_mt_get_progress(self, &mut progress_in, &mut progress_out);
