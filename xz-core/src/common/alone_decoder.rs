...
pub(crate) unsafe fn lzma_alone_decoder_init(
    ...
    if coder.is_null() {
        ...
        (*next).code = coder_code_fn!(alone_decode, lzma_alone_coder);
        (*next).end = coder_end_fn!(alone_decoder_end, lzma_alone_coder);
        (*next).memconfig = coder_memconfig_fn!(alone_decoder_memconfig, lzma_alone_coder);
